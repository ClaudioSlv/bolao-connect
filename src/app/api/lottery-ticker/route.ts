...
async function getLatest(lottery: SupportedLottery) {
  ...
  return {
    ...
    secondDrawNumbers: data.listaDezenasSegundoSorteio ?? [],
    trevos: data.listaTrevos ?? [],
...
export async function GET() {
  ...
  const results = settled.filter((item): item is PromiseFulfilledResult<Awaited<ReturnType<typeof getLatest>>> => item.status === "fulfilled").map((item) => item.value).filter((item) => item.contest > 0);
  return NextResponse.json({ results, updatedAt: new Date().toISOString() }, { headers: { "Cache-Control": "public, max-age=60, s-maxage=300, stale-while-revalidate=300" } });
}
