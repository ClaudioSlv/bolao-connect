...
export async function POST(request: Request) {
  ...
  if (!handle)
    return NextResponse.json(
      { error: "A InfinitePay ainda não está configurada neste aplicativo." },
      { status: 503 },
    );
  ...
  const orderNsu = `bolao-${p.id}-${crypto.randomUUID().slice(0, 12)}`,
  ...
  if (insertError)
    return NextResponse.json(
      {
        error:
          "Já existe uma cobrança sendo criada. Aguarde alguns segundos e tente novamente.",
      },
      { status: 409 },
    );
      ...
      items: [
        {
          quantity: 1,
          price: due,
          description: p.is_test
            ? "Teste Bolão Connect - R$ 1,00"
            : `${p.shares} cota(s) - ${pool.title} - ${p.name}`,
        },
      ],
      order_nsu: orderNsu,
  ...
  const response = (await payment.json().catch(() => null)) as Record<
      string,
      unknown
    > | null,
    checkoutUrl = response?.url || response?.checkout_url || response?.link;
  if (!payment.ok || typeof checkoutUrl !== "string") {
    await s
      .from("payment_checkout_sessions")
      .update({
        status: "failed",
        failure_reason: "provider_checkout_failed",
