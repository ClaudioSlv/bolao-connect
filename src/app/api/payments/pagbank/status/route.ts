...
export async function POST(request: Request) {
  ...
  const response = await fetchPagBank(
  ...
  if (!response.ok || !order) {
    ...
    return NextResponse.json(
      { error: "Não foi possível consultar o Pix." },
      { status: 502 },
    );
  }
  if (!pagBankOrderPaid(order))
    return NextResponse.json({ paid: false, status: "pending" });
  const charge = Array.isArray(order.charges)
      ? order.charges.find(
          (x: any) => String(x?.status).toUpperCase() === "PAID",
        )
      : null,
    transactionId = String(charge?.id || session.provider_order_id);
  const { data: claimed } = await s
    .from("payment_checkout_sessions")
    .update({
      status: "processing",
      transaction_nsu: transactionId,
      paid_amount_cents: session.expected_amount_cents,
  ...
  if (!pool) {
    await s
      .from("payment_checkout_sessions")
      .update({ status: "review_required", failure_reason: "pool_not_found" })
      .eq("id", session.id);
    return NextResponse.json(
      { error: "Pagamento recebido; bolão não encontrado para atualização." },
      { status: 409 },
    );
  }
  if (creditUsed && p.phone) {
    ...
    if (!account || Number(account.balance_cents) < creditUsed) {
      await s
        ...
        .eq("id", session.id);
      return NextResponse.json(
        { error: "O saldo de crédito mudou. O organizador precisa revisar." },
        { status: 409 },
      );
    }
  }
  const { data: payment, error } = await s
    .from("payments")
    .insert({
      pool_id: p.pool_id,
      participant_id: p.id,
  ...
  if (error) {
    await s
      ...
      .update({
        status: "review_required",
        failure_reason: error.code || "payment_insert_failed",
      })
      .eq("id", session.id);
    await logAppError({
      source: "PagBank - registrar pagamento",
      message: error.message,
      poolId: p.pool_id,
      details: { participant_id: p.id, session_id: session.id },
    });
    return NextResponse.json(
      { error: "Pagamento recebido; atualização em revisão." },
      { status: 500 },
    );
  }
  try {
    if (!session.is_test) {
      await s
        ...
        .insert({
          ...
          description: "Pagamento confirmado automaticamente via PagBank",
    ...
    if (!session.is_test && creditUsed) {
      await s
        .from("participant_credit_accounts")
        .update({
          balance_cents: Number(account.balance_cents) - creditUsed,
          updated_at: new Date().toISOString(),
        })
        .eq("id", account.id);
      await s
        ...
        .insert({
          ...
          description: "Crédito usado no pagamento via PagBank",
          created_by: pool.owner_id,
        });
    }
    await s
    ...
    await s
      ...
      .eq("id", session.id);
    await logAppError({
      source: "PagBank - atualizar financeiro",
      message: errorMessage(caught),
      poolId: p.pool_id,
      details: { participant_id: p.id, session_id: session.id },
    });
    return NextResponse.json(
      { error: "Pagamento recebido; atualização financeira em revisão." },
      { status: 500 },
    );
  }
  await s
  ...
  try {
    ...
    await logAppError({
      source: "Notificação - pagamento PagBank",
      message: errorMessage(error),
      poolId: p.pool_id,
      details: { participant_id: p.id },
    });
    console.error("Falha no push PagBank", error);
  }
  return NextResponse.json({ paid: true });
}
