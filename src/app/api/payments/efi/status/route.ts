...
export async function POST(request: Request) {
  ...
  if (error) {
    await s
      ...
      .eq("id", session.id);
    await logAppError({
      source: "Efí - registrar pagamento",
  ...
  try {
    if (!session.is_test) {
      await s
        ...
        .insert({
          ...
          description: "Pagamento confirmado automaticamente via Efí",
    ...
    if (!session.is_test && creditUsed) {
      ...
      await s
        ...
        .insert({
          ...
          description: "Crédito usado no pagamento via Efí",
          created_by: pool.owner_id,
        });
    }
    await s
    ...
    await s
      ...
      .eq("id", session.id);
    await logAppError({
      source: "Efí - atualizar financeiro",
  ...
  try {
    ...
    await logAppError({
      source: "Notificação - pagamento Efí",
