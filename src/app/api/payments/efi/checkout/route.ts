...
export async function POST(request:Request){
 try{
  ...
  const {data:p}=await s.from("participants").select("id,pool_id,name,phone,shares,status,payment_status,payment_deadline_override").eq("access_token",token).maybeSingle();
  ...
  if(opens&&now<opens)return NextResponse.json({error:"Os pagamentos deste bolão ainda não foram abertos."},{status:403});
  if(closes&&now>closes)return NextResponse.json({error:"O prazo de pagamento foi encerrado."},{status:403});
  ...
  const gross=Number(p.shares)*Number(pool.share_price_cents);let credit=0;
  if(p.phone){const {data:a}=await s.from("participant_credit_accounts").select("balance_cents").eq("owner_id",pool.owner_id).eq("phone",digits(p.phone)).maybeSingle();credit=Number(a?.balance_cents||0)}
  ...
  const {error:ie}=await s.from("payment_checkout_sessions").insert({pool_id:pool.id,participant_id:p.id,provider:"efi",order_nsu:referenceId,gross_amount_cents:gross,credit_used_cents:creditUsed,expected_amount_cents:due,status:"creating",is_test:false});
