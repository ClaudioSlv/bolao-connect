...
export async function POST(request:Request){
  ...
  const s=createAdminClient(),{data:session}=await s.from("payment_checkout_sessions").select("id,pool_id,participant_id,order_nsu,gross_amount_cents,credit_used_cents,expected_amount_cents,status").eq("provider","infinitepay").eq("order_nsu",payload.order_nsu).maybeSingle();
  ...
  const{data:payment,error:paymentError}=await s.from("payments").insert({pool_id:session.pool_id,participant_id:p.id,amount_cents:cashPaid,gross_amount_cents:gross,credit_used_cents:creditUsed,status:"confirmed",payment_method:"pix",confirmed_at:new Date().toISOString(),provider:"infinitepay",provider_reference:session.order_nsu,provider_transaction_nsu:payload.transaction_nsu}).select().single();
  ...
  try{
    ...
    const{error:w}=await s.from("wallet_transactions").insert({pool_id:session.pool_id,payment_id:payment.id,type:"payment_confirmed",amount_cents:cashPaid,shares:Number(p.shares),description:"Pagamento confirmado automaticamente via InfinitePay",created_by:pool.owner_id});if(w)throw w;
    if(creditUsed){const{error:cw}=await s.from("wallet_transactions").insert({pool_id:session.pool_id,payment_id:payment.id,type:"credit_used",amount_cents:creditUsed,shares:0,description:`Crédito abatido de ${p.name}`,created_by:pool.owner_id});if(cw)throw cw}
  ...
  await s.from("audit_events").insert({pool_id:session.pool_id,actor_id:pool.owner_id,event_type:"payment_confirmed_automatically",entity_type:"payment",entity_id:payment.id,details:{participant_id:p.id,participant_name:p.name,provider:"infinitepay",order_nsu:session.order_nsu,transaction_nsu:payload.transaction_nsu,gross_amount_cents:gross,cash_paid_cents:cashPaid,credit_used_cents:creditUsed}});
