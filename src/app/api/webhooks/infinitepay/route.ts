...
export async function POST(request: Request) {
  ...
  const s = createAdminClient(),
    { data: session } = await s
      .from("payment_checkout_sessions")
      .select(
        "id,pool_id,participant_id,order_nsu,gross_amount_cents,credit_used_cents,expected_amount_cents,status,is_test",
      )
  ...
  const checked = (await verification.json().catch(() => null)) as {
    paid?: boolean;
    amount?: number;
  } | null;
  if (
    !verification.ok ||
    !checked?.paid ||
    Number(checked.amount) !== Number(session.expected_amount_cents)
  )
    return NextResponse.json(
      { error: "Pagamento ainda não confirmado ou valor diferente." },
      { status: 400 },
    );
  const { data: claimed } = await s
    ...
    .update({
      ...
      paid_amount_cents: checked.amount,
      updated_at: new Date().toISOString(),
    })
    .eq("id", session.id)
    .in("status", ["creating", "pending", "failed"])
    .select("id")
    .maybeSingle();
  if (!claimed) {
  ...
  const { data: p } = await s
    ...
    { data: pool } = await s
      ...
      .maybeSingle();
  if (!p || !pool || p.status !== "confirmed") {
    await s
      .from("payment_checkout_sessions")
      .update({ status: "failed", failure_reason: "participant_unavailable" })
      .eq("id", session.id);
    return NextResponse.json(
  ...
  if (creditUsed && p.phone) {
    const { data } = await s
      ...
      .maybeSingle();
    account = data;
    if (!account || Number(account.balance_cents) < creditUsed) {
      await s
        .from("payment_checkout_sessions")
        .update({
          status: "review_required",
          failure_reason: "credit_balance_changed",
        })
        .eq("id", session.id);
      return NextResponse.json(
        { error: "O saldo de crédito mudou. Revisão necessária." },
        { status: 409 },
      );
    }
  }
  const { data: payment, error: paymentError } = await s
    ...
    .insert({
      ...
      provider_reference: session.order_nsu,
      provider_transaction_nsu: payload.transaction_nsu,
  ...
  if (paymentError) {
    await s
      ...
      .update({
        status: "failed",
        failure_reason: paymentError.code || "payment_insert_failed",
      })
      .eq("id", session.id);
    await logAppError({
      source: "InfinitePay - registrar pagamento",
      message: paymentError.message,
      poolId: session.pool_id,
      details: { participant_id: p.id, session_id: session.id },
    });
    return NextResponse.json(
  ...
  try {
    if (account && creditUsed) {
      ...
      const { error: u } = await s
        .from("participant_credit_accounts")
        .update({
          balance_cents: balance - creditUsed,
          updated_at: new Date().toISOString(),
        })
        .eq("id", account.id)
    ...
    if (!session.is_test) {
      const { error: w } = await s
        ...
        .insert({
          ...
          description: "Pagamento confirmado automaticamente via InfinitePay",
          created_by: pool.owner_id,
        });
      if (w) throw w;
    }
    if (!session.is_test && creditUsed) {
      const { error: cw } = await s
        .from("wallet_transactions")
        .insert({
          pool_id: session.pool_id,
          payment_id: payment.id,
    ...
    await s
      ...
      .update({
        status: "review_required",
        failure_reason: "financial_write_failed",
      })
      .eq("id", session.id);
    await logAppError({
      source: "InfinitePay - atualizar financeiro",
      message: errorMessage(error),
      poolId: session.pool_id,
      details: { participant_id: p.id, session_id: session.id },
    });
    return NextResponse.json(
      { error: "Não foi possível concluir a atualização financeira." },
      { status: 500 },
    );
  }
  await s
  ...
  try {
    const pushResult = await sendPaymentConfirmedPush({
      participantId: p.id,
      amountCents: cashPaid,
    });
    console.log(
      "Resultado do push de pagamento",
      JSON.stringify({ participantId: p.id, ...pushResult }),
    );
  } catch (error) {
    await logAppError({
      source: "Notificação - pagamento InfinitePay",
      message: errorMessage(error),
      poolId: session.pool_id,
      details: { participant_id: p.id },
    });
