...
export async function GET(req:Request){
  ...
  if(!prelaunchError){
    ...
    for(const sub of prelaunchSubs??[]){
      if(sub.campaign_key!==NEXT_POOL_PRELAUNCH.key||!Number.isFinite(opensAt)||opensAt<=now){
        await s.from("prelaunch_push_subscriptions").update({enabled:false,updated_at:new Date().toISOString()}).eq("id",sub.id);
        prelaunchDisabled++;
        continue;
      }
      const anchor=sub.last_sent_at||sub.created_at;
