...
export async function GET(req:Request){
  ...
  const {data:subs,error}=await s.from("push_subscriptions").select("id,pool_id,participant_id,endpoint,p256dh,auth,last_sent_at").eq("enabled",true).limit(500);
  ...
  for(const sub of subs??[]){
    ...
    if(!p||p.status==="cancelled"||p.payment_status==="confirmed"){await s.from("push_subscriptions").update({enabled:false,updated_at:new Date().toISOString()}).eq("id",sub.id);disabled++;continue}
    ...
    if(sub.last_sent_at&&now-new Date(sub.last_sent_at).getTime()<10*24*60*60*1000){skipped++;continue}
    ...
    try{
      ...
      await s.from("push_subscriptions").update({last_sent_at:new Date().toISOString(),updated_at:new Date().toISOString()}).eq("id",sub.id);sent++;
    }catch(e:any){
      if(e?.statusCode===404||e?.statusCode===410){await s.from("push_subscriptions").update({enabled:false,updated_at:new Date().toISOString()}).eq("id",sub.id);disabled++}
    }
  }
