...
export default async function Home({searchParams}:{searchParams:Promise<{pool?:string}>}){
  ...
  return <main className="shell">
    ...
    <section className="section"><h2>Gerenciar {pool?pool.title:"bolão"}</h2><div className="grid"><Link className="card" href="/organizador/perfil"><strong>🎨 Minha identidade</strong><span>Nome da marca e domínio futuro</span></Link><Link className="card" href={`/carteira${q}`}><strong>💰 Carteira</strong><span>Arrecadação transparente</span></Link><Link className="card" href={`/jogos${q}`}><strong>🎟️ Jogos</strong><span>Apostas e comprovantes</span></Link><Link className="card" href={`/conferencia${q}`}><strong>✅ Conferência</strong><span>Resultados e acertos</span></Link><Link className="card" href={`/participantes${q}`}><strong>👥 Participantes</strong><span>Lista atualizada automaticamente</span></Link><Link className="card" href="/meu-jogo"><strong>🎲 Meu Jogo</strong><span>Escolha números ou gere combinações</span></Link><Link className="card" href={timerHref}><strong>⏳ Temporizador</strong><span>{pool?.public_slug?"Link principal para compartilhar":"Pré-abertura do próximo bolão"}</span></Link>{pool?.public_slug?<Link className="card" href={`/bolao/${pool.public_slug}`}><strong>🔗 Link do bolão</strong><span>Informações do bolão</span></Link>:null}</div></section>
