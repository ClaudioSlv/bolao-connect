...
export default async function TestClient({params}:{params:Promise<{token:string}>}){
  ...
  return <main className="shell"><Link className="back" href="/teste-pagamento">← Voltar</Link><section className="section"><p className="eyebrow">MODO DE TESTE · PAGBANK</p><h1>🧪 {p.name}</h1><p className="muted">{pool?.title}</p><div className="card test-amount"><span>Valor do teste Sandbox</span><strong>R$ 1,00</strong><span>Não ocupa cota e não entra na arrecadação do bolão.</span></div></section>{paid?<section className="section"><div className="test-result-paid">✓ TESTE PAGO</div><p className="status">✅ RETORNO AUTOMÁTICO CONFIRMADO</p><p className="muted">O PagBank identificou a cobrança e o aplicativo atualizou este cliente automaticamente.</p></section>:<section className="section"><h2>Gerar cobrança</h2><PagBankCheckout token={token} amountLabel="R$ 1,00"/></section>}</main>;
}
