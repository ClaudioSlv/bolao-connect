  ...
  try {
    ...
    const now = Date.now(),
      opens = pool.payment_opens_at
        ...
        : 0,
      closes = new Date(pool.payment_deadline).getTime(),
      draw = pool.draw_at ? new Date(pool.draw_at).getTime() : 0;
    const beforeOpen = opens > now,
      windowOpen = !beforeOpen && now < closes,
      afterClose = now >= closes;
    const planned = Number(pool.planned_games) || (games?.length ?? 0),
      numbers =
        Number(pool.numbers_per_game) ||
        (Array.isArray(games?.[0]?.numbers) ? games![0].numbers.length : 0);
    const canJoin = pool.status === "open" && !afterClose;
    return (
      <main className="shell">
        <ScrollPageTop />
          ...
          {beforeOpen ? (
            <>
              <p className="eyebrow" style={{ textAlign: "center" }}>
                FALTAM PARA ABRIR OS PAGAMENTOS
              </p>
              <PoolCountdown target={pool.payment_opens_at} />
              <p className="muted" style={{ textAlign: "center" }}>
                🔒 Pix e comprovante ficam fechados até{" "}
            ...
            <>
              ...
              {draw > now ? (
                <>
                  <h2 style={{ textAlign: "center" }}>Falta para o sorteio</h2>
                  <PoolCountdown target={pool.draw_at} />
                </>
              ) : (
                <p className="muted" style={{ textAlign: "center" }}>
                  O prazo de pagamento deste bolão terminou.
                </p>
              )}
            </>
          )}
        </section>
        {beforeOpen && <TimerReminderOptIn slug={slug} />}
        <section className="section">
          <div className="wallet">
            {pool.estimated_prize_cents ? (
              <div className="wallet-row">
                <span>🏆 Prêmio estimado</span>
                <strong>{money(Number(pool.estimated_prize_cents))}</strong>
              </div>
            ) : null}
            <div className="wallet-row">
            ...
            <div className="wallet-row">
              <span>🎟️ Cotas ocupadas</span>
              <strong>
                {used} / {pool.total_shares}
              </strong>
            </div>
            <div className="wallet-row">
            ...
            <div className="wallet-row">
              <span>🎯 Jogos planejados</span>
              <strong>{planned}</strong>
            </div>
            <div className="wallet-row">
              <span>🧾 Jogos cadastrados</span>
              <strong>
                {games?.length ?? 0} / {planned}
              </strong>
            </div>
            {numbers ? (
              <div className="wallet-row">
            ...
            <div className="wallet-row">
              ...
              <strong>{money(Number(pool.share_price_cents))}</strong>
            </div>
            <div className="wallet-row">
              <span>💳 Pagamento</span>
              <strong>
            ...
            {pool.draw_at ? (
              <div className="wallet-row">
                <span>📅 Sorteio</span>
                <strong>
                  {new Date(pool.draw_at).toLocaleString("pt-BR", {
                    timeZone: "America/Sao_Paulo",
                  })}
                </strong>
              </div>
        ...
        <section className="section" style={{ textAlign: "center" }}>
          {canJoin ? (
            <>
              ...
              <p className="muted">
                {available > 0
                  ? `Reserve 1 ou 2 cotas agora. ${beforeOpen ? "O pagamento será liberado automaticamente na data indicada." : "Os pagamentos já estão abertos."}`
                  : "As cotas principais foram preenchidas. Novos participantes podem entrar na lista de espera e só pagam se uma cota for liberada."}
              </p>
              <JoinPoolButton
                href={`/bolao/${slug}`}
                label={
    ...
    return (
      <main className="shell">
        <section className="section">
          <h1>Temporizador indisponível</h1>
          <p className="muted">Não foi possível carregar este bolão.</p>
        </section>
      </main>
    );
  }
