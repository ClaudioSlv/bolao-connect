...
export default async function TimerPage({params}:{params:Promise<{slug:string}>}){
  ...
  const {data:pool}=await s.from("pools").select("id,title,lottery,contest_number,estimated_prize_cents,share_price_cents,total_shares,payment_deadline,draw_at,status,public_slug").eq("public_slug",slug).maybeSingle();
