...
async function submitReceipt(f:FormData){
  ...
  const {data:pool}=await s.from("pools").select("rules_version,public_slug,payment_deadline").eq("id",p.pool_id).maybeSingle();
  if(pool?.public_slug===MEGA_SLUG&&Date.now()<new Date(pool.payment_deadline).getTime())throw new Error("Os pagamentos deste bolão ainda não foram abertos.");
...
export default async function Page({params,searchParams}:{params:Promise<{token:string}>;searchParams:Promise<{sent?:string}>}){
  ...
  try{const s=createAdminClient();const {data:p}=await s.from("participants").select("id,pool_id,name,shares,status,payment_status,selfie_path,selfie_consent_at").eq("access_token",token).maybeSingle();if(p){const {data:pool}=await s.from("pools").select("title,lottery,share_price_cents,payment_deadline,public_slug,rules_text,rules_version").eq("id",p.pool_id).maybeSingle();const {data:sub}=await s.from("payment_submissions").select("status,created_at").eq("participant_id",p.id).eq("status","pending").order("created_at",{ascending:false}).limit(1).maybeSingle();const version=Number(pool?.rules_version||DEFAULT_POOL_RULES_VERSION);const {data:acceptance}=await s.from("pool_rule_acceptances").select("accepted_at").eq("participant_id",p.id).eq("pool_id",p.pool_id).eq("rules_version",version).maybeSingle();const {data:credits}=await s.from("participant_credits").select("remaining_cents").eq("participant_id",p.id).eq("status","available");data={p,pool,sub,acceptance,credit:(credits??[]).reduce((sum,row)=>sum+Number(row.remaining_cents||0),0)}}}catch{}
  ...
  const isMega2026=pool.public_slug===MEGA_SLUG;
  const paymentOpen=!isMega2026||Date.now()>=new Date(pool.payment_deadline).getTime();
  return <main className="shell"><section className="section"><p className="eyebrow">BOLÃO AMIGOS BTP</p><h1>🍀 {pool.title}</h1><p><strong>{p.name}</strong> · {p.shares} cota(s) · <strong>{money(amount)}</strong></p><p className="muted">{isMega2026?"Abertura dos pagamentos":"Prazo"}: {new Date(pool.payment_deadline).toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"})}</p>{credit>0&&<p className="status">Crédito disponível: {money(credit)}</p>}</section>
  {!paid&&!isMega2026&&<ReminderOptIn token={token}/>}<section className="section"><h2>📜 Regras do Bolão</h2><div className="card"><span style={{whiteSpace:"pre-line"}}>{rules}</span></div>{acceptance?<p className="status">✓ Regras aceitas em {new Date(acceptance.accepted_at).toLocaleString("pt-BR",{timeZone:"America/Sao_Paulo"})}</p>:<form className="form" action={acceptRules}><input type="hidden" name="token" value={token}/><label><input type="checkbox" name="agreed" required/> Li e estou de acordo com as Regras do Bolão.</label><button className="button primary">Aceitar regras e continuar</button></form>}</section>
