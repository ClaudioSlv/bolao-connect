import Link from "next/link";
import { redirect } from "next/navigation";
import { AppNav } from "@/components/app-nav";
import { PoolSwitcher } from "@/components/pool-switcher";
import { CopyParticipantLink } from "@/components/copy-participant-link";
import { ParticipantPaymentForm } from "@/components/participant-payment-form";
import { addParticipant, cancelParticipant } from "@/app/actions/participants";
import { increasePoolCapacity } from "@/app/actions/pools";
import { createClient } from "@/lib/supabase/server";
import { createAdminClient } from "@/lib/supabase/admin";
export const dynamic = "force-dynamic";
async function addF(f: FormData) {
  "use server";
  const poolId = String(f.get("poolId") ?? "");
  await addParticipant({
    poolId,
    name: String(f.get("name") ?? "").trim(),
    phone: String(f.get("phone") ?? "").trim(),
    shares: Number(f.get("shares")),
  });
  redirect(`/participantes?pool=${poolId}`);
}
async function noteF(f: FormData) {
  "use server";
  const poolId = String(f.get("poolId") ?? ""),
    participantId = String(f.get("participantId") ?? ""),
    notes = String(f.get("notes") ?? "").trim();
  const s = await createClient();
  const { data: a } = await s.auth.getUser();
  if (!a.user) return;
  const { data: p } = await s
    .from("pools")
    .select("id")
    .eq("id", poolId)
    .eq("owner_id", a.user.id)
    .maybeSingle();
  if (!p) return;
  await s
    .from("participants")
    .update({ notes: notes || null })
    .eq("id", participantId)
    .eq("pool_id", poolId);
  redirect(`/participantes?pool=${poolId}`);
}
async function cancelF(f: FormData) {
  "use server";
  const poolId = String(f.get("poolId") ?? "");
  await cancelParticipant({
    poolId,
    participantId: String(f.get("participantId") ?? ""),
  });
  redirect(`/participantes?pool=${poolId}`);
}
async function capacityF(f: FormData) {
  "use server";
  const poolId = String(f.get("poolId") ?? "");
  await increasePoolCapacity({
    poolId,
    totalShares: Number(f.get("totalShares")),
  });
  redirect(`/participantes?pool=${poolId}`);
}
function initials(name: string) {
  return (
    name
      .trim()
      .split(/\s+/)
      .slice(0, 2)
      .map((x) => x[0]?.toUpperCase() ?? "")
      .join("") || "?"
  );
}
const money = (c: number) =>
  new Intl.NumberFormat("pt-BR", {
    style: "currency",
    currency: "BRL",
  }).format(c / 100);
export default async function Page({
  searchParams,
}: {
  searchParams: Promise<{ pool?: string }>;
}) {
  const { pool: requested } = await searchParams,
    s = await createClient(),
    { data: a } = await s.auth.getUser();
  if (!a.user)
    return (
      <main className="shell">
        <section className="section">
          <h1>👥 Participantes</h1>
          <p>Entre na sua conta.</p>
        </section>
      </main>
    );
  const { data: all } = await s
      .from("pools")
      .select("id,title,lottery,total_shares,share_price_cents,public_slug")
      .eq("owner_id", a.user.id)
      .order("created_at", { ascending: false }),
    pools = all ?? [],
    pool = pools.find((p) => p.id === requested) ?? pools[0] ?? null;
  const { data: participants } = pool
    ? await s
        .from("participants")
        .select(
          "id,name,phone,shares,payment_status,status,access_token,waitlist_position,notes",
        )
        .eq("pool_id", pool.id)
        .order("created_at", { ascending: true })
    : { data: [] };
  const visible = (participants ?? []).filter((p) => p.status !== "cancelled");
  const ids = visible.map((p) => p.id);
  const { data: paymentRows } = ids.length
    ? await s
        .from("payments")
        .select("participant_id,amount_cents,credit_used_cents,status")
        .in("participant_id", ids)
        .in("status", ["partial", "confirmed"])
    : { data: [] };
  const receivedByParticipant = new Map<string, number>();
  for (const row of paymentRows ?? []) {
    receivedByParticipant.set(
      row.participant_id,
      (receivedByParticipant.get(row.participant_id) ?? 0) +
        Number(row.amount_cents || 0) +
        Number(row.credit_used_cents || 0),
    );
  }
  const { data: subs } = ids.length
    ? await s
        .from("payment_submissions")
        .select("participant_id,status,receipt_path")
        .in("participant_id", ids)
        .eq("status", "pending")
    : { data: [] };
  const admin = createAdminClient();
  const { data: pushSubs } = ids.length
    ? await admin
        .from("push_subscriptions")
        .select("participant_id,enabled")
        .in("participant_id", ids)
        .eq("enabled", true)
    : { data: [] };
  const notificationActive = new Set(
      (pushSubs ?? []).map((x) => x.participant_id),
    ),
    pending = new Set((subs ?? []).map((x) => x.participant_id)),
    receiptUrls = new Map<string, string>();
  if (pool && subs?.length) {
    for (const sub of subs) {
      if (!sub.receipt_path) continue;
      const { data: signed } = await admin.storage
        .from("payment-receipts")
        .createSignedUrl(sub.receipt_path, 600);
      if (signed?.signedUrl)
        receiptUrls.set(sub.participant_id, signed.signedUrl);
    }
  }
  const confirmed = visible.filter((p) => p.status === "confirmed"),
    used = confirmed.reduce((x, p) => x + Number(p.shares || 0), 0),
    waitlist = visible
      .filter((p) => p.status === "waitlisted")
      .sort(
        (a, b) =>
          Number(a.waitlist_position ?? Number.MAX_SAFE_INTEGER) -
          Number(b.waitlist_position ?? Number.MAX_SAFE_INTEGER),
      );
  return (
    <main className="shell participants-shell">
      <style>{`.paid-ticket{display:grid;grid-template-columns:30px 1fr 30px;align-items:stretch;min-height:58px;margin-top:10px;border:2px solid #31f46e;border-radius:12px;overflow:hidden;background:linear-gradient(180deg,#063817,#021b0b);box-shadow:0 0 12px rgba(49,244,110,.42),inset 0 0 12px rgba(49,244,110,.13);color:#fff}.paid-ticket-side{display:flex;align-items:center;justify-content:center;background:linear-gradient(180deg,#075826,#063b1a);color:#bfffd0;font-size:9px;font-weight:900;letter-spacing:.08em;writing-mode:vertical-rl;transform:rotate(180deg);border-right:1px dashed rgba(153,255,181,.55)}.paid-ticket-side:last-child{transform:none;border-right:0;border-left:1px dashed rgba(153,255,181,.55)}.paid-ticket strong{display:flex;align-items:center;justify-content:center;gap:7px;font-size:18px;letter-spacing:.08em;text-shadow:0 0 9px rgba(255,255,255,.55)}.paid-ticket i{display:inline-flex;width:22px;height:22px;align-items:center;justify-content:center;border:2px solid #56ff85;border-radius:50%;color:#56ff85;font-size:13px;font-style:normal;box-shadow:0 0 8px rgba(49,244,110,.55)}.notification-status{display:block;margin-top:7px;font-size:12px;font-weight:800}.notification-on{color:#52f47d}.notification-off{color:#f4c852}.share-count{font-weight:900}.share-count-double{font-weight:900!important;color:#fff!important}@media(max-width:520px){.paid-ticket{grid-template-columns:18px 1fr 18px;min-height:46px;margin-top:7px;border-radius:9px}.paid-ticket-side{font-size:7px}.paid-ticket strong{gap:3px;font-size:11px}.paid-ticket i{width:15px;height:15px;font-size:9px;border-width:1px}.notification-status{font-size:9px}}`}</style>
      <Link className="back" href="/">
        ← Voltar
      </Link>
      <section className="section">
        <h1>👥 Participantes</h1>
        <PoolSwitcher
          pools={pools}
          activeId={pool?.id}
          basePath="/participantes"
        />
        <p className="muted">
          {pool
            ? `${pool.title} · ${used}/${pool.total_shares} cotas confirmadas`
            : "Crie um bolão antes de adicionar participantes."}
        </p>
        {pool && (
          <>
            <form className="form" action={addF}>
              <input type="hidden" name="poolId" value={pool.id} />
              <div className="field">
                <label>Nome</label>
                <input name="name" required />
              </div>
              <div className="field">
                <label>WhatsApp</label>
                <input name="phone" inputMode="tel" />
              </div>
              <div className="field">
                <label>Cotas</label>
                <input
                  name="shares"
                  type="number"
                  min="1"
                  max={Math.min(2, Math.max(1, pool.total_shares - used))}
                  defaultValue="1"
                  required
                />
              </div>
              <button
                className="button primary"
                disabled={used >= pool.total_shares}
              >
                + Adicionar participante
              </button>
            </form>
            <form className="form" action={capacityF}>
              <input type="hidden" name="poolId" value={pool.id} />
              <div className="field">
                <label>➕ Aumentar total de vagas</label>
                <input
                  name="totalShares"
                  type="number"
                  min={Number(pool.total_shares) + 1}
                  max="100000"
                  defaultValue={Number(pool.total_shares) + 1}
                  required
                />
                <span className="muted">
                  Atual: {pool.total_shares}. Ao salvar, o sistema chama
                  automaticamente os primeiros da lista de espera até preencher
                  as novas vagas.
                </span>
              </div>
              <button className="button primary">Salvar novas vagas</button>
            </form>
          </>
        )}
      </section>
      {waitlist.length > 0 && (
        <section className="section">
          <h2>⏳ Lista de espera ({waitlist.length})</h2>
          <p className="muted">
            A ordem abaixo será respeitada automaticamente quando surgirem novas
            vagas.
          </p>
          {waitlist.map((p, i) => (
            <div className="list-item" key={p.id}>
              <strong>
                #{i + 1} · {p.name}
              </strong>
              {p.phone && <span className="muted"> · {p.phone}</span>}
            </div>
          ))}
        </section>
      )}
      <section className="section list participants-grid">
        {visible
          .filter((p) => p.status !== "waitlisted")
          .map((p) => {
            const paid = p.payment_status === "confirmed",
              amount = Number(p.shares) * Number(pool?.share_price_cents ?? 0),
              received = Math.min(
                amount,
                receivedByParticipant.get(p.id) ?? 0,
              ),
              remaining = Math.max(0, amount - received),
              hasReceipt = pending.has(p.id),
              receiptUrl = receiptUrls.get(p.id),
              link = `/p/${p.access_token}`,
              state = paid ? "paid" : "pending",
              notified = notificationActive.has(p.id),
              doubleShare = Number(p.shares) === 2;
            return (
              <div
                className={`list-item participant-card participant-${state}`}
                style={
                  doubleShare
                    ? {
                        border: "3px solid #2196f3",
                        boxShadow:
                          "0 0 14px rgba(33,150,243,.65), inset 0 0 0 1px rgba(33,150,243,.35)",
                      }
                    : undefined
                }
                key={p.id}
              >
                <div className="participant-main">
                  <div className="participant-avatar" aria-hidden="true">
                    {initials(p.name)}
                  </div>
                  <div className="participant-info">
                    <strong>{p.name}</strong>
                    <span
                      className={`muted share-count ${doubleShare ? "share-count-double" : ""}`}
                      style={
                        doubleShare
                          ? { fontWeight: 900, color: "#fff" }
                          : undefined
                      }
                    >
                      {p.shares} cota(s)
                    </span>
                    {p.phone && (
                      <div className="muted participant-phone">{p.phone}</div>
                    )}
                    <span
                      className={`notification-status ${notified ? "notification-on" : "notification-off"}`}
                    >
                      {notified
                        ? "🔔 Notificação ativada"
                        : "🔕 Notificação não ativada"}
                    </span>
                    <div className="actions participant-actions">
                      <Link className="button secondary" href={link}>
                        💳 Abrir participante
                      </Link>
                      <CopyParticipantLink token={p.access_token} />
                      {receiptUrl && !paid && (
                        <a
                          className="button secondary"
                          href={receiptUrl}
                          target="_blank"
                          rel="noreferrer"
                        >
                          🧾 Comprovante
                        </a>
                      )}
                    </div>
                    {hasReceipt && !paid && (
                      <div className="status">COMPROVANTE RECEBIDO</div>
                    )}
                  </div>
                </div>
                <div className="participant-controls">
                  {pool && (
                    <form className="form participant-note-form" action={noteF}>
                      <input type="hidden" name="poolId" value={pool.id} />
                      <input type="hidden" name="participantId" value={p.id} />
                      <div className="field">
                        <label>Observação</label>
                        <input
                          name="notes"
                          defaultValue={p.notes ?? ""}
                          placeholder="Ex.: cota da mãe do Fábio"
                        />
                      </div>
                      <button className="button secondary">💾 Salvar</button>
                    </form>
                  )}
                  {paid ? (
                    <div className="paid-ticket" aria-label="Cota paga">
                      <span className="paid-ticket-side">COTA</span>
                      <strong>
                        <i>✓</i>PAGO<i>✓</i>
                      </strong>
                      <span className="paid-ticket-side">COTA</span>
                    </div>
                  ) : pool ? (
                    <>
                      {received > 0 && (
                        <div className="status" style={{ marginBottom: 8 }}>
                          PARCIAL: {money(received)} pago · falta {money(remaining)}
                        </div>
                      )}
                      <ParticipantPaymentForm
                        poolId={pool.id}
                        participantId={p.id}
                        shares={Number(p.shares)}
                        remainingCents={remaining}
                      />
                    </>
                  ) : null}
                  {!paid && pool && (
                    <form className="participant-cancel-form" action={cancelF}>
                      <input type="hidden" name="poolId" value={pool.id} />
                      <input type="hidden" name="participantId" value={p.id} />
                      <button className="button">Cancelar</button>
                    </form>
                  )}
                </div>
              </div>
            );
          })}
        {!visible.length && (
          <p className="muted">Nenhum participante cadastrado.</p>
        )}
      </section>
      <AppNav />
    </main>
  );
}
