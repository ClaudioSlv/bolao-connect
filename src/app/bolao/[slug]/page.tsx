...
export default async function PublicPool({
  params,
}: {
    ...
    return (
      <main className="shell">
        <section className="section">
          ...
          <p className="muted">Confira se o link recebido está correto.</p>
        </section>
      </main>
    );
  ...
  const [
    ...
    s
      .from("pools")
      .select("planned_games,numbers_per_game")
      .eq("id", pool.id)
      .maybeSingle(),
  ]);
  ...
  const inferredNumbers = Array.isArray(games?.[0]?.numbers)
    ...
    <main className="shell">
      ...
      <section className="section">
        ...
        <div className="wallet">
          ...
          <div className="wallet-row">
            <span>Jogos</span>
            <strong>{plannedGames || games?.length || 0}</strong>
          </div>
          {numbersPerGame ? (
            <div className="wallet-row">
              <span>Dezenas por jogo</span>
              <strong>{numbersPerGame}</strong>
            </div>
          ) : null}
          <div className="wallet-row">
      ...
      <section className="section">
        ...
        <div className="wallet">
          ...
          <div className="pool-progress-head">
            <span>{progress}% arrecadado</span>
            <span>
              {money(collected)} de {money(target)}
            </span>
          </div>
          <div
            className="pool-progress"
            role="progressbar"
            aria-label={`${progress}% arrecadado`}
            aria-valuemin={0}
            aria-valuemax={100}
      ...
      <section className="section">
        {registrationOpen ? (
          <>
            ...
            <p className="muted">
              {isFull
                ? "As cotas principais estão preenchidas. Você pode entrar na lista de espera por ordem de inscrição e só deverá pagar se ganhar uma cota."
                : "Cada participante pode reservar 1 ou 2 cotas neste bolão. O número de participantes e a quantidade de cotas são contabilizados separadamente."}
            </p>
            <JoinPoolButton
              href={`/bolao/${slug}/entrar`}
              label={
                isFull
          ...
          <>
            ...
            <p className="muted">
              {deadlinePassed
                ? "O prazo de participação deste bolão terminou."
                : "Este bolão não está aberto para novas participações."}
            </p>
          </>
        )}
      </section>
