...
async function confirmDevice(formData: FormData) {
  ...
  const verifyHref = `/verificar-participante/${encodeURIComponent(token)}`;
  ...
  if (attempts?.locked_until && new Date(attempts.locked_until).getTime() > Date.now()) {
    redirect(`${verifyHref}?erro=bloqueado`);
  }
  ...
  if (last4.length !== 4 || phone.length < 4 || phone.slice(-4) !== last4) {
    ...
    redirect(`${verifyHref}?erro=${lockedUntil ? "bloqueado" : "digitos"}`);
  }
  ...
  if (error) {
    redirect(`${verifyHref}?erro=sessao`);
  }
