...
export function LotteryResultsTicker(){
 ...
 return <aside className="lottery-ticker" aria-label="Últimos resultados das Loterias CAIXA"><div className="lottery-ticker-label"><img src="https://commons.wikimedia.org/wiki/Special:Redirect/file/Loterias%20Caixa%20logo%202017.svg" alt="Loterias CAIXA"/><span>Resultados CAIXA</span></div><div className="lottery-ticker-window"><div className="lottery-ticker-track">{items.map((r,index)=><div className={`lottery-ticker-item ${tones[r.lottery]??""}`} key={`${r.lottery}-${index}`}><strong>{r.label}</strong>{r.contest>0?<><span>Concurso {r.contest}</span>{r.numbers.length>0&&<span className="ticker-numbers">{r.numbers.join(" • ")}</span>}{r.secondDrawNumbers.length>0&&<span>2º sorteio: <b>{r.secondDrawNumbers.join(" • ")}</b></span>}{r.trevos.length>0&&<span>Trevos: <b>{r.trevos.join(" • ")}</b></span>}{r.special&&<span><b>{r.special}</b></span>}<span className={r.accumulated?"ticker-accumulated":""}>{r.accumulated?"ACUMULOU":"Resultado"}</span>{r.nextPrize>0&&<span>Próximo prêmio: <b>{money.format(r.nextPrize)}</b></span>}</>:<span>{waitingText}</span>}<i>◆</i></div>)}</div></div></aside>;
}
