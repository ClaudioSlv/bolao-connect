...
export function LotteryResultsTicker(){
 ...
 useEffect(()=>{let cancelled=false;async function load(){try{const response=await fetch("/api/lottery-ticker",{cache:"no-store"});if(!response.ok)throw new Error("ticker api");const data=(await response.json())as{results?:Result[]};if(!cancelled&&Array.isArray(data.results)&&data.results.length){setResults(data.results);setOnline(true)}}catch{if(!cancelled)setOnline(false)}}load();const timer=window.setInterval(load,5*60*1000);return()=>{cancelled=true;window.clearInterval(timer)}},[]);
 const items=[...results,...results];
 return <aside className="lottery-ticker" aria-label="Últimos resultados das Loterias CAIXA"><div className="lottery-ticker-label">Resultados CAIXA</div><div className="lottery-ticker-window"><div className="lottery-ticker-track">{items.map((r,index)=><div className={`lottery-ticker-item ${tones[r.lottery]??""}`} key={`${r.lottery}-${index}`}><strong>{r.label}</strong>{r.contest>0?<><span>Concurso {r.contest}</span>{r.numbers.length>0&&<span className="ticker-numbers">{r.numbers.join(" • ")}</span>}{r.secondDrawNumbers.length>0&&<span>2º sorteio: <b>{r.secondDrawNumbers.join(" • ")}</b></span>}{r.trevos.length>0&&<span>Trevos: <b>{r.trevos.join(" • ")}</b></span>}{r.special&&<span><b>{r.special}</b></span>}<span className={r.accumulated?"ticker-accumulated":""}>{r.accumulated?"ACUMULOU":"Resultado"}</span>{r.nextPrize>0&&<span>Próximo prêmio: <b>{money.format(r.nextPrize)}</b></span>}</>:<span>{online?"Atualizando…":"Buscando resultado…"}</span>}<i>◆</i></div>)}</div></div></aside>;
}
