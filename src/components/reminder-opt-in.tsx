...
export function ReminderOptIn({token,paid=false}:{token:string;paid?:boolean}){
  ...
  return <div className="section"><h2>🔔 Notificações do bolão</h2><p className="muted">{paid?"Sua cota já está quitada. Ative as notificações para receber avisos dos jogos, resultados, prêmios, créditos e outras novidades do bolão. Você não receberá lembretes de pagamento.":"Ative as notificações para receber avisos deste bolão e lembretes enquanto seu pagamento estiver pendente."}</p><button className="button secondary" type="button" disabled={state==="busy"} onClick={enable}>{state==="busy"?"Ativando...":"🔔 ATIVAR NOTIFICAÇÕES NESTE CELULAR"}</button>{state==="error"&&<p className="status" style={{color:"#facc15"}}>⚠️ Não foi possível ativar. Verifique a permissão de notificações deste site no navegador e toque no botão novamente.</p>}</div>
}
