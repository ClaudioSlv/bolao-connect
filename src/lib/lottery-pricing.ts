...
const SUPER_SETE_PRICES: Record<number, number> = {
  7: 300, 8: 600, 9: 1200, 10: 2400, 11: 4800, 12: 9600, 13: 19200,
  14: 38400, 15: 57600, 16: 86400, 17: 129600, 18: 194400,
...
export function calculatePoolPricing(lottery: LotteryId, rawPlan: GamePlanInput[], participants: number, prices?: LotteryPriceMap) {
  ...
  if (!Array.isArray(rawPlan) || rawPlan.length < 1 || rawPlan.length > 30)
    throw new Error("Adicione pelo menos um tipo de jogo.");
  const items: GamePlanItem[] = rawPlan.map((row) => {
    const quantity = Number(row.quantity), numbers = Number(row.numbers);
  ...
  return { items, totalCostCents, totalGames, sharePriceCents, projectedCollectionCents,
    roundingDifferenceCents, uniformNumbers: uniqueNumbers.length === 1 ? uniqueNumbers[0] : undefined };
}

function isLotteryId(value: string): value is LotteryId {
